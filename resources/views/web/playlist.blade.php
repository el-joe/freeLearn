            ...
            @foreach ($lessons as $lesson)
                <div class="flex flex-col gap-10 bg-white shadow-xl p-9 rounded-lg cursor-pointer">
                    <div data-aos="fade-left" class="sm:w-full relative mt-10 sm:mt-0">
                        <a href="{{route('video',[$lesson->id])}}"><img class="rounded-xl relative" src="{{$lesson->thumb?->file_path}}" alt="" /></a>
                        <a href="{{route('video',[$lesson->id])}}" class="bg-white w-14 h-14 rounded-full flex items-center justify-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 focus:outline-none transform transition hover:scale-110 duration-300 ease-in-out z-0">
                    ...
                    <a href="{{route('video',[$lesson->id])}}"  data-aos="fade-right" class="relative">
